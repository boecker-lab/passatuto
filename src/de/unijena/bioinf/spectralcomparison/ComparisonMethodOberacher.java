...
public class ComparisonMethodOberacher implements ComparisonMethod{

	static public final String methodNameLong="Oberacher";
	ParametersSpectralComparison p;
	public List<MassBank> s1;
	public List<MassBank> s2;


	public ComparisonMethodOberacher(ParametersSpectralComparison p) {
		this.p=p;
	}
	...
	public Result getResult(List<double[]> peaksLeft, List<double[]> peaksRight){
		...
		Result r=new Result(numberMatches, 1.0*Math.pow(numberMatches,2)/peaksLeft.size()/peaksRight.size());
		return r;
	}
