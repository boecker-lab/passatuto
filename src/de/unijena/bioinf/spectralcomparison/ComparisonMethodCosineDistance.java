...
public class ComparisonMethodCosineDistance implements ComparisonMethod{

	static public final String methodNameLong="CosineDistance";
	ParametersSpectralComparison p;
	public List<MassBank> s1;
	public List<MassBank> s2;


	public ComparisonMethodCosineDistance(ParametersSpectralComparison p) {
		this.p=p;
	}
	...
	public Result getResult(List<double[]> peaksLeft, List<double[]> peaksRight){
		...
		for(double[] p:peaksLeft){
			p1.add(new double[]{p[0],p[1]/maxLeft});
		}
		List<double[]> p2=new ArrayList<double[]>();
		for(double[] p:peaksRight){
			p2.add(new double[]{p[0],p[1]/maxRight});
		}
