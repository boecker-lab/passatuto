...
public class ComparisonMethodOberacherWithIntensities implements ComparisonMethod{

	static public final String methodNameLong="OberacherWithIntensities";
	ParametersSpectralComparison p;
	public List<MassBank> s1;
	public List<MassBank> s2;


	public ComparisonMethodOberacherWithIntensities(ParametersSpectralComparison p) {
		this.p=p;
	}
	...
	public Result getResult(List<double[]> peaksLeft, List<double[]> peaksRight){
		...
		for(double[] p:peaksLeft)p1.add(new double[]{p[0], p[1]/maxLeft*100});
		List<double[]> p2=new ArrayList<double[]>();
		for(double[] p:peaksRight)p2.add(new double[]{p[0], p[1]/maxRight*100});
		...
		Result r=new Result(numberMatches, numberMatches!=0?1.0*Math.pow(numberMatches,4)/p1.size()/p2.size()/Math.pow(intensityDiff,0.25):0);
		return r;
	}
