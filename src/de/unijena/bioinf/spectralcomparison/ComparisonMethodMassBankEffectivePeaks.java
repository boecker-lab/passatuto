...
public class ComparisonMethodMassBankEffectivePeaks implements ComparisonMethod{

	static public final String methodNameLong="MassBankEffectivePeaks";
	ParametersSpectralComparison p;
	public List<MassBank> s1;
	public List<MassBank> s2;


	public ComparisonMethodMassBankEffectivePeaks(ParametersSpectralComparison p) {
		this.p=p;
	}
	...
	public Result getResult(List<double[]> peaksLeft, List<double[]> peaksRight){
		...
		double length2=0;
		int numberMatches=0;
		int numberEffectivePeaksQuery=0;
		int numberEffectiveMatchesQuery=0;
		...
		List<Boolean> isEffectivePeakQuery=new ArrayList<Boolean>();
		for(double[] p:peaksLeft){
			isEffectivePeakQuery.add(1000*p[1]/maxLeft>=5&&p[0]<1000);
			p1.add(new double[]{p[0],Math.pow(1000*p[1]/maxLeft,0.5)*p[0]/10});
		}
		List<double[]> p2=new ArrayList<double[]>();
		for(double[] p:peaksRight)p2.add(new double[]{p[0],Math.pow(1000*p[1]/maxRight,0.5)*p[0]/10});
		
		for(double[] p:p1)length1+=Math.pow(p[1],2);
		for(double[] p:p2)length2+=Math.pow(p[1],2);

		for(int i=0;i<p1.size();i++){
			double[] peaks1=p1.get(i);
			...
			if(bestMatchingPeak!=null){
				if(isEffectivePeakQuery.get(i))numberEffectiveMatchesQuery++;
				cos+=peaks1[1]*bestMatchingPeak[1];
				numberMatches++;
			}
		}
		
		for(Boolean b:isEffectivePeakQuery)if(b)numberEffectivePeaksQuery++;
		
		if((numberEffectivePeaksQuery>=3&&numberMatches<3)||(numberEffectivePeaksQuery<3&&numberEffectivePeaksQuery!=numberEffectiveMatchesQuery)){
			numberMatches=0;
			cos=0;
		}
